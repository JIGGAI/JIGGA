...
- ⏭️ **W6 — file-backed mailbox** (`inbox/`/`outbox/`): durable, greppable agent-to-agent (and human-to-agent) messages, complementing the structured handoff decision log with ad-hoc notes. File-first, shows up in `jigga trace`. The remaining half of the coordination direction (handoffs = structured routing; mailbox = free-form). Design first; append-only + auditable, no ephemeral bus.
